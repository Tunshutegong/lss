package com.jiehun.component.widgets.glideimageview.progress;
...
public class CircleProgressView extends ProgressBar {
    ...
    private int mReachBarSize = AbDisplayUtil.dip2px(2); // 未完成进度条大小
    private int mNormalBarSize = AbDisplayUtil.dip2px(2); // 未完成进度条大小
    private int mReachBarColor = Color.parseColor("#108ee9"); // 已完成进度颜色
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA"); // 未完成进度颜色
    private int mTextSize = AbDisplayUtil.sp2px(14); // 进度值字体大小
    ...
    private int mOuterSize = AbDisplayUtil.dip2px(1); // 外层圆环宽度
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = AbDisplayUtil.dip2px(reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = AbDisplayUtil.dip2px(normalBarSize);
        invalidate();
    }
    ...
    public void setTextSize(int textSize) {
        mTextSize = AbDisplayUtil.sp2px(textSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = AbDisplayUtil.dip2px(radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = AbDisplayUtil.dip2px(innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = AbDisplayUtil.dip2px(outerSize);
        invalidate();
    }
