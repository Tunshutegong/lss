...
public class RecyclerAdapterWithHF extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    @RecycleAdapterState
    private int state = RecycleAdapterState.State.STATE_NORMAL;//空白頁功能
    ...
    public void setState(@RecycleAdapterState int state) {
        this.state = state;
    ...
    public final int getItemCount() {
        if (RecycleAdapterState.State.STATE_EMPTY == state) {
            return mHeaders.size();
        }
    ...
    // add a header to the adapter
    public void addHeader(View header, int size) {
        if (!mHeaders.contains(header)) {
            mHeaders.add(size,header);
            // animate
            notifyItemInserted(mHeaders.size() - 1);
        }
    }
    ...
    // our header/footer RecyclerView.ViewRecycleHolder is just a FrameLayout
